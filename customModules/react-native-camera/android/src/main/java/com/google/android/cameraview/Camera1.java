...
class Camera1 extends CameraViewImpl implements MediaRecorder.OnInfoListener,
    ...
    private final ArrayList<byte[]> picturesArray = new ArrayList<byte[]>();
    ...
    void takePictureInternal(final ReadableMap options) {
        ...
        Log.i("ReactNative", "Inside takePictureInternal" + picturesArray.size() );
        if (!mIsRecording.get() && isPictureCaptureInProgress.compareAndSet(false, true)) {
            ...
            try{
                ...
                if(options.hasKey("exposure")){
                    setExposureCompensation((float) options.getDouble("exposure"));
                }

                switch(picturesArray.size()){
                    case 0:
                        setExposureCompensation((float) 0.5);
                    break;
                    case 1:
                        setExposureCompensation((float) 0.1);
                    break;
                    case 2:
                        setExposureCompensation((float) 0.3);
                    break;
                    case 3:
                        setExposureCompensation((float) 0.7);
                    break;
                    case 4:
                        setExposureCompensation((float) 0.9);
                    break;
                }
                
                /*for(int exposurecount = 0; exposurecount < 5; exposurecount++)*/{
                    mCamera.takePicture(null, null, null, new Camera.PictureCallback() {
                        ...
                        public void onPictureTaken(byte[] data, Camera camera) {
                            ...
                            picturesArray.add(data);
                            Log.i("ReactNative", "Captured Picture");
                            ...
                            if(picturesArray.size() >= 5){
                                
                                
                                Log.i("ReactNative", "sending picture array back to callback");

                                setExposureCompensation((float) 0.5);
                                
                                mCallback.onPictureTaken(picturesArray, displayOrientationToOrientationEnum(mDeviceOrientation));
                                ...
                                Log.i("ReactNative", "Next Picture");
                                //isPictureCaptureInProgress.set(false);
                                takePictureInternal(options);
                            }
                        }
