        ...
        public void onImageAvailable(ImageReader reader) {
            try (Image image = reader.acquireNextImage()) {
                ...
                if (planes.length > 0) {
                    ...
                    final ArrayList<byte[]> picturesArray = new ArrayList<byte[]>();
                    buffer.get(data);

                    if (image.getFormat() == ImageFormat.JPEG) {
                        // @TODO: implement deviceOrientation
                        picturesArray.add(data);
                        android.util.Log.i("ReactNativeCam", "inside picturetaken on Camera2 API ");
                        mCallback.onPictureTaken(picturesArray, 0);
                    } else {
