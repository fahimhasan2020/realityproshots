...
- (id)initWithBridge:(RCTBridge *)bridge
{
    if ((self = [super init])) {
        ...
        self.previewLayer.videoGravity = AVLayerVideoGravityResizeAspect; //AVLayerVideoGravityResizeAspectFill;
...
- (void)_prepareBracketsWithCompletionHandler
{
    NSMutableArray * bracketedStillSetting = [[NSMutableArray alloc] init];
    
    [bracketedStillSetting addObject:[AVCaptureAutoExposureBracketedStillImageSettings autoExposureSettingsWithExposureTargetBias:-2.0]];
    [bracketedStillSetting addObject:[AVCaptureAutoExposureBracketedStillImageSettings autoExposureSettingsWithExposureTargetBias:-1.0]];
    [bracketedStillSetting addObject:[AVCaptureAutoExposureBracketedStillImageSettings autoExposureSettingsWithExposureTargetBias:1.0]];
    [bracketedStillSetting addObject:[AVCaptureAutoExposureBracketedStillImageSettings autoExposureSettingsWithExposureTargetBias:2.0]];
    
    AVCaptureConnection *connection = [self.stillImageOutput connectionWithMediaType:AVMediaTypeVideo];
    [self.stillImageOutput prepareToCaptureStillImageBracketFromConnection:connection
                                                     withSettingsArray:bracketedStillSetting
                                                     completionHandler:^(BOOL prepared, NSError *error) {
        [self updateSessionPreset:AVCaptureSessionPresetPhoto];
        RCTLog(@"prepareToCaptureStillImageBracketFromConnection");
    }];
}

- (void)takePictureBracket:(NSDictionary *)options resolve:(RCTPromiseResolveBlock)resolve reject:(RCTPromiseRejectBlock)reject
{
    ...
    __block int exposurecounter = bracketedStillSetting.count;
    ...
    @try {
        ...
        [self.stillImageOutput captureStillImageBracketAsynchronouslyFromConnection:connection withSettingsArray:bracketedStillSetting completionHandler:^(CMSampleBufferRef  imageSampleBuffer, AVCaptureBracketedStillImageSettings  *imageSettings, NSError *error) {
            
            AVCaptureAutoExposureBracketedStillImageSettings * setting = (AVCaptureAutoExposureBracketedStillImageSettings*)imageSettings;
            RCTLog(@"captureStillImageBracketAsynchronouslyFromConnection  counter %d -- exposure %f", exposurecounter, setting.exposureTargetBias);
            exposurecounter -= 1;
            if (exposurecounter >= 0) {
                RCTLog(@"capture multiple completed");
            ...
            if (imageSampleBuffer && !error) {
                ...
                if (CGImageDestinationFinalize(destination)){
                    ...
                    if (useFastMode) {
                        ...
                        [picturesArray addObject:response];
                        if(exposurecounter == 0)
                            resolve(picturesArray);
                    }
                }
                else{
...
- (void)takePicture:(NSDictionary *)options resolve:(RCTPromiseResolveBlock)resolve reject:(RCTPromiseRejectBlock)reject
{
    ...
    /*if ([self.session canSetSessionPreset:AVCaptureSessionPresetPhoto]) {
        RCTLog(@"The selected preset [%@]", AVCaptureSessionPresetPhoto);
        ...
        RCTLog(@"The selected preset [%@] does not work with the current session.", AVCaptureSessionPresetPhoto);
    }*/
 

    NSInteger orientation = [options[@"orientation"] integerValue];

    CGFloat exposure = [options[@"exposure"] floatValue];
    ...
    if(exposure == 1.0){
        printf("calling takePictureBracket");
        [self takePictureBracket:options resolve:resolve reject:reject];
        return;
    }

    else{
        printf("calling simple picture");
        AVCaptureConnection *connection = [self.stillImageOutput connectionWithMediaType:AVMediaTypeVideo];
        [connection setVideoOrientation:orientation];
...
- (void)startSession
{
    ...
    dispatch_async(self.sessionQueue, ^{
        ...
        if ([self.session canAddOutput:stillImageOutput]) {
            ...
            //self.session.isHighResolutionCaptureEnabled = true;
        ...
        [self _prepareBracketsWithCompletionHandler];
    });
}
...
- (void)initializeCaptureSessionInput
{
    ...
    dispatch_async(self.sessionQueue, ^{
        ...
        self.session.sessionPreset = AVCaptureSessionPresetPhoto; //[self getDefaultPreset];
...
- (void)updateSessionPreset:(AVCaptureSessionPreset)preset
{
    ...
    if (preset) {
        ...
        RCTLog(@"Setting The selected preset [%@] .", preset);
        dispatch_async(self.sessionQueue, ^{
